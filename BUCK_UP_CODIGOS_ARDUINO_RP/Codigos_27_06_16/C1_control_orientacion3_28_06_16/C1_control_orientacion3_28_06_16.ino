...
///******************
long  t_mag = 0; //Tiempo para el magnetometro
...
void loop(){
    ...
    if(yaw_v > 360)
     ...
     if(flag_control == true){
       ...
       //if(abs(beta-yaw_v)<50){ //Cercanía entre angulos
         if(beta >=0 && beta < 20){
           rot_coord = 60;
         }
...
//       }
       ...
       if(phi>360) phi-=360; //Cuando se hace negativo, se aumenta una vuelta
...
void RTK_recepcion(){
 ...
 if(RTK.available()>0){
      ...
      latitude = integracion_gps(datas[0],datas[1],datas[2],datas[3]);
      longitude = integracion_gps(datas[4],datas[5],datas[6],datas[7]);
...
//  if(abs(e)>=15){
//    vl = 0; //giro rapido, solo w en los WP
//    maximo_rpm = 15;
//  }
//  else{ //Velocidad lineal cuando ya llego al angulo, ahí controla v-w
//    vl = 0.3;
//    maximo_rpm = 40;
//  }
