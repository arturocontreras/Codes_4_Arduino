...
// //Aplicar el Filtro Kalman
 ...
 Angle[0]  = kalman1.getAngle(ang_x, hxx, (double)(micros() - timer1)/1000000); // Calculate the angle using a Kalman filter
 timer1 = micros();
 
 Angle[1]  = kalman2.getAngle(ang_y, hyy, (double)(micros() - timer2)/1000000); // Calculate the angle using a Kalman filter
 timer2 = micros();

 Angle[0] = Angle[0]+90;
 Angle[1] = Angle[1]+90;

 Serial.print((char)Angle[0]);
 Serial.print((char)Angle[1]);
 delay(50); 
}
//---------------- Functions
